const guessingGame = {
    ...
    matchLetterHistory: function(username, gameinfo) {
        return `<ol class="matchLetterList">` +
    ...
    getGuessScore: function(username, gamescore) {
        return `
          <p>Score(Valid guesses you have made): ${gamescore[username]}</p>
        `;
    },

    guessSubmitResult: function(username, gamestatus) {
        if(gamestatus[username]["InputStatus"] === "invalid") {
            return `<p>Invalid guess! Try again.</p>`;
        }else if(gamestatus[username]["InputStatus"] === "correct") {
            return `<p>Bingo! Your guess "${gamestatus[username]["mostRecentGuess"]}" is correct!</p>`;
        }else if(gamestatus[username]["InputStatus"] === "valid") {
            return `<p>Your guess is "${gamestatus[username]["mostRecentGuess"]}", and it matches ${gamestatus[username]["lettersMatch"]} letters. Try again!</p>`;
        }else {
